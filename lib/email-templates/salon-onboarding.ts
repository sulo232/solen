...
export function onboardingWelcome(to: string, vars: OnboardingVars, locale: EmailLocale = "de") {
  ...
  const bodies: Record<EmailLocale, string> = {
    ...
    it: `<p>Ciao,</p><p>Benvenuto su solen.ch! <strong>${vars.salonName}</strong> fa ora parte della nostra piattaforma.</p><p><a href="https://solen.ch/it/dashboard">Vai alla dashboard →</a></p>`,
  };
...
export function onboardingCompleteProfile(to: string, vars: OnboardingVars, locale: EmailLocale = "de") {
  const subjects: Record<EmailLocale, string> = {
    ...
    it: `Completa il tuo profilo, ${vars.salonName}`,
  };
  const bodies: Record<EmailLocale, string> = {
    ...
    it: `<p>Il tuo profilo non è ancora completo. Più informazioni aggiungi, più facilmente i clienti ti troveranno.</p><p><a href="https://solen.ch/it/dashboard/settings">Modifica profilo →</a></p>`,
  };
  return { to, subject: subjects[locale], html: bodies[locale] };
}
...
export function onboardingAddServices(to: string, vars: OnboardingVars, locale: EmailLocale = "de") {
  const subjects: Record<EmailLocale, string> = {
    ...
    it: `Aggiungi i tuoi servizi, ${vars.salonName}`,
  };
  const bodies: Record<EmailLocale, string> = {
    ...
    it: `<p>Non hai ancora aggiunto servizi. I clienti possono prenotare solo quando i servizi sono disponibili.</p><p><a href="https://solen.ch/it/dashboard/services">Aggiungi servizi →</a></p>`,
  };
  return { to, subject: subjects[locale], html: bodies[locale] };
}
