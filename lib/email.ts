...
export function walkInPaymentEmail(
  ...
  const bodies: Record<EmailLocale, string> = {
    de: `<p>Hallo ${vars.customerName},</p><p>Bitte bezahlen Sie <strong>${vars.amount}</strong> für <strong>${vars.serviceName}</strong> bei <strong>${vars.salonName}</strong>:</p><p><a href="${vars.paymentUrl}" style="display:inline-block;padding:12px 24px;background:#C05038;color:#fff;border-radius:8px;text-decoration:none;font-weight:600">Jetzt bezahlen →</a></p><p>solen.ch</p>`,
    en: `<p>Hello ${vars.customerName},</p><p>Please pay <strong>${vars.amount}</strong> for <strong>${vars.serviceName}</strong> at <strong>${vars.salonName}</strong>:</p><p><a href="${vars.paymentUrl}" style="display:inline-block;padding:12px 24px;background:#C05038;color:#fff;border-radius:8px;text-decoration:none;font-weight:600">Pay now →</a></p><p>solen.ch</p>`,
    fr: `<p>Bonjour ${vars.customerName},</p><p>Veuillez payer <strong>${vars.amount}</strong> pour <strong>${vars.serviceName}</strong> chez <strong>${vars.salonName}</strong> :</p><p><a href="${vars.paymentUrl}" style="display:inline-block;padding:12px 24px;background:#C05038;color:#fff;border-radius:8px;text-decoration:none;font-weight:600">Payer maintenant →</a></p><p>solen.ch</p>`,
    it: `<p>Ciao ${vars.customerName},</p><p>Si prega di pagare <strong>${vars.amount}</strong> per <strong>${vars.serviceName}</strong> presso <strong>${vars.salonName}</strong>:</p><p><a href="${vars.paymentUrl}" style="display:inline-block;padding:12px 24px;background:#C05038;color:#fff;border-radius:8px;text-decoration:none;font-weight:600">Paga ora →</a></p><p>solen.ch</p>`,
  };
  return { to, subject: subjects[locale], html: bodies[locale] };
}
...
export function tipPromptEmail(
  ...
  const bodies: Record<EmailLocale, string> = {
    de: `<p>Hallo ${vars.customerName},</p><p>Waren Sie zufrieden mit Ihrem Termin? Hinterlassen Sie ein Trinkgeld für <strong>${vars.stylistName}</strong>!</p>${photoHtml}<p><a href="${vars.tipUrl}" style="display:inline-block;padding:12px 24px;background:#C05038;color:#fff;border-radius:8px;text-decoration:none;font-weight:600">Trinkgeld geben →</a></p>`,
    en: `<p>Hello ${vars.customerName},</p><p>Happy with your appointment? Leave a tip for <strong>${vars.stylistName}</strong>!</p>${photoHtml}<p><a href="${vars.tipUrl}" style="display:inline-block;padding:12px 24px;background:#C05038;color:#fff;border-radius:8px;text-decoration:none;font-weight:600">Leave a tip →</a></p>`,
    fr: `<p>Bonjour ${vars.customerName},</p><p>Satisfait(e) de votre rendez-vous ? Laissez un pourboire à <strong>${vars.stylistName}</strong> !</p>${photoHtml}<p><a href="${vars.tipUrl}" style="display:inline-block;padding:12px 24px;background:#C05038;color:#fff;border-radius:8px;text-decoration:none;font-weight:600">Laisser un pourboire →</a></p>`,
    it: `<p>Ciao ${vars.customerName},</p><p>Soddisfatto del tuo appuntamento? Lascia una mancia a <strong>${vars.stylistName}</strong>!</p>${photoHtml}<p><a href="${vars.tipUrl}" style="display:inline-block;padding:12px 24px;background:#C05038;color:#fff;border-radius:8px;text-decoration:none;font-weight:600">Lascia una mancia →</a></p>`,
  };
  return { to, subject: subjects[locale], html: bodies[locale] };
}
...
export function giftCardDeliveryEmail(
  ...
  const bodies: Record<EmailLocale, string> = {
    de: `<p>Hallo ${vars.recipientName},</p><p><strong>${vars.senderName}</strong> hat dir eine Geschenkkarte im Wert von <strong>${vars.amount}</strong> auf solen.ch geschenkt!</p>${msgHtml}<p style="text-align:center;margin:20px 0"><span style="font-family:monospace;font-size:24px;letter-spacing:3px;background:#FAF6EF;padding:12px 20px;border-radius:8px;border:2px dashed #C05038;display:inline-block">${vars.code}</span></p><p>Verwende diesen Code bei deiner nächsten Buchung auf <a href="https://solen.ch">solen.ch</a>.</p>`,
    en: `<p>Hello ${vars.recipientName},</p><p><strong>${vars.senderName}</strong> sent you a gift card worth <strong>${vars.amount}</strong> on solen.ch!</p>${msgHtml}<p style="text-align:center;margin:20px 0"><span style="font-family:monospace;font-size:24px;letter-spacing:3px;background:#FAF6EF;padding:12px 20px;border-radius:8px;border:2px dashed #C05038;display:inline-block">${vars.code}</span></p><p>Use this code on your next booking at <a href="https://solen.ch">solen.ch</a>.</p>`,
    fr: `<p>Bonjour ${vars.recipientName},</p><p><strong>${vars.senderName}</strong> vous a offert une carte cadeau d'une valeur de <strong>${vars.amount}</strong> sur solen.ch !</p>${msgHtml}<p style="text-align:center;margin:20px 0"><span style="font-family:monospace;font-size:24px;letter-spacing:3px;background:#FAF6EF;padding:12px 20px;border-radius:8px;border:2px dashed #C05038;display:inline-block">${vars.code}</span></p><p>Utilisez ce code lors de votre prochaine réservation sur <a href="https://solen.ch">solen.ch</a>.</p>`,
    it: `<p>Ciao ${vars.recipientName},</p><p><strong>${vars.senderName}</strong> ti ha regalato una carta regalo del valore di <strong>${vars.amount}</strong> su solen.ch!</p>${msgHtml}<p style="text-align:center;margin:20px 0"><span style="font-family:monospace;font-size:24px;letter-spacing:3px;background:#FAF6EF;padding:12px 20px;border-radius:8px;border:2px dashed #C05038;display:inline-block">${vars.code}</span></p><p>Usa questo codice per la tua prossima prenotazione su <a href="https://solen.ch">solen.ch</a>.</p>`,
  };
  return { to, subject: subjects[locale], html: bodies[locale] };
}
...
export function nailInfillReminderEmail(
  ...
  const bodies: Record<EmailLocale, string> = {
    de: `<p>Hallo ${vars.customerName},</p><p>Dein letzter <strong>${vars.serviceName}</strong>-Termin bei <strong>${vars.salonName}</strong> war am ${vars.lastVisitDate}. Es ist Zeit für eine Auffüllung!</p><p><a href="${vars.bookingUrl}" style="display:inline-block;padding:12px 24px;background:#C05038;color:#fff;border-radius:8px;text-decoration:none;font-weight:600">Jetzt Termin buchen →</a></p><p>solen.ch</p>`,
    en: `<p>Hello ${vars.customerName},</p><p>Your last <strong>${vars.serviceName}</strong> appointment at <strong>${vars.salonName}</strong> was on ${vars.lastVisitDate}. Time for an infill!</p><p><a href="${vars.bookingUrl}" style="display:inline-block;padding:12px 24px;background:#C05038;color:#fff;border-radius:8px;text-decoration:none;font-weight:600">Book now →</a></p><p>solen.ch</p>`,
    fr: `<p>Bonjour ${vars.customerName},</p><p>Votre dernier rendez-vous <strong>${vars.serviceName}</strong> chez <strong>${vars.salonName}</strong> était le ${vars.lastVisitDate}. C'est l'heure du remplissage !</p><p><a href="${vars.bookingUrl}" style="display:inline-block;padding:12px 24px;background:#C05038;color:#fff;border-radius:8px;text-decoration:none;font-weight:600">Réserver maintenant →</a></p><p>solen.ch</p>`,
    it: `<p>Ciao ${vars.customerName},</p><p>Il tuo ultimo appuntamento <strong>${vars.serviceName}</strong> presso <strong>${vars.salonName}</strong> era il ${vars.lastVisitDate}. È ora del ritocco!</p><p><a href="${vars.bookingUrl}" style="display:inline-block;padding:12px 24px;background:#C05038;color:#fff;border-radius:8px;text-decoration:none;font-weight:600">Prenota ora →</a></p><p>solen.ch</p>`,
  };
  return { to, subject: subjects[locale], html: bodies[locale] };
}
...
export function barberSmartReminderEmail(
  ...
  const bodies: Record<EmailLocale, string> = {
    de: `<p>Hey ${vars.customerName},</p><p>Dein letzter Schnitt bei <strong>${vars.salonName}</strong> war vor ${vars.daysSince} Tagen. Bereit für ein frisches Styling?</p><p><a href="${vars.bookingUrl}" style="display:inline-block;padding:12px 24px;background:#C05038;color:#fff;border-radius:8px;text-decoration:none;font-weight:600">Jetzt Termin buchen →</a></p><p style="color:#999;font-size:12px;margin-top:24px">— Dein Solen Team</p>`,
    en: `<p>Hey ${vars.customerName},</p><p>Your last cut at <strong>${vars.salonName}</strong> was ${vars.daysSince} days ago. Ready for a fresh look?</p><p><a href="${vars.bookingUrl}" style="display:inline-block;padding:12px 24px;background:#C05038;color:#fff;border-radius:8px;text-decoration:none;font-weight:600">Book now →</a></p><p style="color:#999;font-size:12px;margin-top:24px">— Your Solen Team</p>`,
    fr: `<p>Salut ${vars.customerName},</p><p>Ta dernière coupe chez <strong>${vars.salonName}</strong> remonte à ${vars.daysSince} jours. Prêt pour un nouveau look ?</p><p><a href="${vars.bookingUrl}" style="display:inline-block;padding:12px 24px;background:#C05038;color:#fff;border-radius:8px;text-decoration:none;font-weight:600">Réserver maintenant →</a></p><p style="color:#999;font-size:12px;margin-top:24px">— Ton équipe Solen</p>`,
    it: `<p>Ciao ${vars.customerName},</p><p>Il tuo ultimo taglio da <strong>${vars.salonName}</strong> risale a ${vars.daysSince} giorni fa. Pronto per un nuovo look?</p><p><a href="${vars.bookingUrl}" style="display:inline-block;padding:12px 24px;background:#C05038;color:#fff;border-radius:8px;text-decoration:none;font-weight:600">Prenota ora →</a></p><p style="color:#999;font-size:12px;margin-top:24px">— Il tuo team Solen</p>`,
  };
  return { to, subject: subjects[locale], html: bodies[locale] };
}
...
export function barberLoyaltyRewardEmail(
  ...
  const bodies: Record<EmailLocale, string> = {
    de: `<p>Hey ${vars.customerName},</p><p>Glückwunsch! Deine Treuekarte bei <strong>${vars.salonName}</strong> ist voll. Du hast dir folgende Belohnung verdient:</p><p style="background:#FAF6EF;padding:16px;border-radius:8px;text-align:center;font-size:18px;font-weight:600;color:#C05038">${vars.reward}</p><p><a href="${vars.redeemUrl}" style="display:inline-block;padding:12px 24px;background:#C05038;color:#fff;border-radius:8px;text-decoration:none;font-weight:600">Belohnung einlösen →</a></p><p style="color:#999;font-size:12px;margin-top:24px">— Dein Solen Team</p>`,
    en: `<p>Hey ${vars.customerName},</p><p>Congrats! Your loyalty card at <strong>${vars.salonName}</strong> is complete. You've earned:</p><p style="background:#FAF6EF;padding:16px;border-radius:8px;text-align:center;font-size:18px;font-weight:600;color:#C05038">${vars.reward}</p><p><a href="${vars.redeemUrl}" style="display:inline-block;padding:12px 24px;background:#C05038;color:#fff;border-radius:8px;text-decoration:none;font-weight:600">Redeem reward →</a></p><p style="color:#999;font-size:12px;margin-top:24px">— Your Solen Team</p>`,
    fr: `<p>Salut ${vars.customerName},</p><p>Félicitations ! Ta carte fidélité chez <strong>${vars.salonName}</strong> est complète. Tu as gagné :</p><p style="background:#FAF6EF;padding:16px;border-radius:8px;text-align:center;font-size:18px;font-weight:600;color:#C05038">${vars.reward}</p><p><a href="${vars.redeemUrl}" style="display:inline-block;padding:12px 24px;background:#C05038;color:#fff;border-radius:8px;text-decoration:none;font-weight:600">Utiliser la récompense →</a></p><p style="color:#999;font-size:12px;margin-top:24px">— Ton équipe Solen</p>`,
    it: `<p>Ciao ${vars.customerName},</p><p>Complimenti! La tua carta fedeltà da <strong>${vars.salonName}</strong> è completa. Hai guadagnato:</p><p style="background:#FAF6EF;padding:16px;border-radius:8px;text-align:center;font-size:18px;font-weight:600;color:#C05038">${vars.reward}</p><p><a href="${vars.redeemUrl}" style="display:inline-block;padding:12px 24px;background:#C05038;color:#fff;border-radius:8px;text-decoration:none;font-weight:600">Riscatta il premio →</a></p><p style="color:#999;font-size:12px;margin-top:24px">— Il tuo team Solen</p>`,
  };
  return { to, subject: subjects[locale], html: bodies[locale] };
}
...
export function tosUpdateNotification(
  ...
  const bodies: Record<EmailLocale, string> = {
    de: `<p>Guten Tag,</p><p>Wir haben unsere Nutzungsbedingungen und unsere Datenschutzerklärung aktualisiert. Die Änderungen treten am <strong>${vars.effectiveDate}</strong> in Kraft.</p><p><a href="${vars.termsUrl}" style="display:inline-block;padding:12px 24px;background:#C05038;color:#fff;border-radius:8px;text-decoration:none;font-weight:600">Neue AGB lesen →</a></p><p>Bitte loggen Sie sich ein, um die neuen Bedingungen zu akzeptieren.</p><p>Mit freundlichen Grüssen,<br>Das solen.ch Team</p>`,
    en: `<p>Hello,</p><p>We have updated our Terms of Service and Privacy Policy. The changes take effect on <strong>${vars.effectiveDate}</strong>.</p><p><a href="${vars.termsUrl}" style="display:inline-block;padding:12px 24px;background:#C05038;color:#fff;border-radius:8px;text-decoration:none;font-weight:600">Read new Terms →</a></p><p>Please log in to accept the new terms.</p><p>Best regards,<br>The solen.ch Team</p>`,
    fr: `<p>Bonjour,</p><p>Nous avons mis à jour nos Conditions Générales et notre Politique de Confidentialité. Les modifications entrent en vigueur le <strong>${vars.effectiveDate}</strong>.</p><p><a href="${vars.termsUrl}" style="display:inline-block;padding:12px 24px;background:#C05038;color:#fff;border-radius:8px;text-decoration:none;font-weight:600">Lire les nouvelles conditions →</a></p><p>Veuillez vous connecter pour accepter les nouvelles conditions.</p><p>Cordialement,<br>L'équipe solen.ch</p>`,
    it: `<p>Buongiorno,</p><p>Abbiamo aggiornato i nostri Termini di Servizio e l'Informativa sulla Privacy. Le modifiche entrano in vigore il <strong>${vars.effectiveDate}</strong>.</p><p><a href="${vars.termsUrl}" style="display:inline-block;padding:12px 24px;background:#C05038;color:#fff;border-radius:8px;text-decoration:none;font-weight:600">Leggi i nuovi Termini →</a></p><p>Accedi per accettare i nuovi termini.</p><p>Cordiali saluti,<br>Il team solen.ch</p>`,
  };
  return { to, subject: subjects[locale], html: bodies[locale] };
}
