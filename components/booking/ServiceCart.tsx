    ...
    <div className="rounded-[12px] border border-s-ink/[0.06] bg-[--raised] p-5 space-y-4"
      ...
      {services.map(svc => (
        <div key={svc.id} className="space-y-2">
          <div className="flex justify-between text-sm">
            ...
            <span className="data-text font-bold text-base text-s-ink">
          ...
          {addons[svc.id]?.map(addon => (
            <label key={addon.id}
              className="flex items-center gap-3 py-2.5 px-3 rounded-[10px] cursor-pointer hover:bg-s-bg-base:bg-s-dm-bg transition-colors duration-150"
      ...
      <div className="space-y-2 pt-3 border-t border-s-ink/[0.06]">
        ...
        <div className="relative">
          <Ticket size={13} className="absolute left-3 top-1/2 -translate-y-1/2 text-s-ink/30" />
          <input
            type="text"
            value={promoCode}
