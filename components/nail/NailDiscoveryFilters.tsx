...
interface NailDiscoveryFiltersProps {
  ...
  onStyleChange: (v: NailStyleCategory | null) => void;
  onShapeChange: (v: NailShape | null) => void;
  onMaterialChange: (v: NailMaterial | null) => void;
}

const STYLES: { value: NailStyleCategory; label: string }[] = [
...
const MATERIALS: { value: NailMaterial; label: string }[] = [
  { value: "gel", label: "Gel" },
    ...
    <div className="flex gap-1.5 overflow-x-auto scrollbar-hide">
      {options.map((opt) => (
        <button
          ...
          className={`shrink-0 text-xs px-3 py-1.5 rounded-pill border transition-colors ${
            value === opt.value
