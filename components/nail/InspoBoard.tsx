...
export default function InspoBoard({ open, onClose, onSelect }: InspoBoardProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center">
      ...
      {/* Modal */}
      <div className="relative w-full sm:max-w-lg max-h-[80vh] bg-white dark:bg-s-dm-surface rounded-t-2xl sm:rounded-card shadow-xl flex flex-col overflow-hidden">
        {/* Header */}
        <div className="flex items-center justify-between px-4 py-3 border-b border-s-ink/5 dark:border-s-dm-text/10">
          <h3 className="font-heading font-semibold text-s-ink dark:text-s-dm-text">Meine Inspo Boards</h3>
          <button onClick={onClose} className="p-1 text-s-ink/40 dark:text-s-dm-text/40 hover:text-s-ink">
            <X size={18} />
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-4">
          {loading ? (
            <p className="text-center text-sm text-s-ink/40 dark:text-s-dm-text/40 py-8">Laden...</p>
          ) : images.length === 0 ? (
            <div className="text-center py-8">
              <FolderOpen size={32} className="mx-auto text-s-ink/20 dark:text-s-dm-text/20 mb-2" />
              <p className="text-sm text-s-ink/40 dark:text-s-dm-text/40">Noch keine Bilder gespeichert</p>
            </div>
          ) : (
        ...
        {selected.size > 0 && (
          <div className="px-4 py-3 border-t border-s-ink/5 dark:border-s-dm-text/10">
            ...
            >
              {selected.size} Bild{selected.size !== 1 ? "er" : ""} auswählen
            </button>
          </div>
        )}
      </div>
