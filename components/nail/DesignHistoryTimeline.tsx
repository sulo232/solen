...
export default function DesignHistoryTimeline({ customerId, salonId, locale = "de" }: DesignHistoryTimelineProps) {
  ...
  if (loading) return <div className="py-4 text-center text-sm text-s-ink/40 dark:text-s-dm-text/40">Laden...</div>;
  if (designs.length === 0) return <p className="text-sm text-s-ink/40 dark:text-s-dm-text/40 py-4">Noch keine Designs</p>;
            ...
            <div className="rounded-card border border-s-ink/5 dark:border-s-dm-text/10 bg-white dark:bg-s-dm-surface p-3">
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-1.5 text-xs text-s-ink/40 dark:text-s-dm-text/40 mb-1">
                    <Clock size={10} />
                    {new Date(d.created_at).toLocaleDateString("de-CH", { day: "numeric", month: "short", year: "numeric" })}
                  </div>
              ...
              <div className="flex gap-2 mt-2 pt-2 border-t border-s-ink/5 dark:border-s-dm-text/10">
                ...
                {d.image_url && salonId && (
                  ...
                  >
                    <Share2 size={10} />
                    {publishingId === d.id ? "..." : "Auf Discovery"}
                  </button>
                )}
              </div>
            </div>
