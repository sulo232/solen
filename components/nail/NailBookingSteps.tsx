    ...
    <div className="space-y-4 py-4 border-t border-b border-s-ink/5 dark:border-s-dm-text/10">
      ...
      {serviceMaterialType ? (
        <div className="flex items-center gap-2 text-sm text-s-ink/60 dark:text-s-dm-text/60">
          <span>Material: <strong className="text-s-ink dark:text-s-dm-text">{serviceMaterialType}</strong></span>
        </div>
      ) : (
      ...
      {repeatLast && (
        ...
        >
          ...
          <div>
            <p className="text-sm font-medium text-s-ink dark:text-s-dm-text">Letztes Design wiederholen</p>
      ...
      {tierLabel && tierPrice != null && (
        <div className="flex items-center justify-between p-3 rounded-card bg-s-bg-surface dark:bg-s-dm-bg">
          <span className="text-sm text-s-ink/70 dark:text-s-dm-text/70">
            {tierLabel.charAt(0).toUpperCase() + tierLabel.slice(1)} Nail Artist
          </span>
          <span className="font-medium text-s-ink dark:text-s-dm-text">
            CHF {(tierPrice / 100).toFixed(2)}
          </span>
        </div>
      )}
      ...
      {stationInfo && stationInfo.total > 0 && (
        <div className="flex items-center gap-2 text-xs text-s-ink/50 dark:text-s-dm-text/50">
          <Users size={12} />
          Noch {stationInfo.available} von {stationInfo.total} Plätzen verfügbar
        </div>
      )}
    </div>
  );
