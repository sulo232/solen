...
export default function InspoUploader({ bookingId, onImagesChange, onOpenBoard }: InspoUploaderProps) {
  ...
  return (
    <div>
      <p className="text-sm font-medium text-s-ink dark:text-s-dm-text mb-2">Inspiration hochladen</p>
      ...
      >
        ...
        <p className="text-xs text-s-ink/40 dark:text-s-dm-text/40 mb-3">
          Bilder hierhin ziehen oder:
        </p>
        <div className="flex justify-center gap-2">
          <button
            type="button"
            onClick={() => fileRef.current?.click()}
            className="flex items-center gap-1.5 px-3 py-1.5 text-xs rounded-button border border-s-ink/10 dark:border-s-dm-text/10 text-s-ink/70 dark:text-s-dm-text/70 hover:border-s-coral/30 transition-colors"
          >
            <Upload size={12} />
            Datei wählen
          </button>
          ...
          <button
            type="button"
            onClick={onOpenBoard}
            className="flex items-center gap-1.5 px-3 py-1.5 text-xs rounded-button bg-s-coral/10 text-s-coral hover:bg-s-coral/20 transition-colors"
          >
            <FolderOpen size={12} />
            Aus Board wählen
          </button>
      ...
      {previews.length > 0 && (
        <div className="flex gap-2 mt-3 overflow-x-auto">
          {previews.map((p, i) => (
            <div key={i} className="relative shrink-0 w-16 h-16 rounded-button overflow-hidden">
