            ...
            <div>
              ...
              <div className="flex gap-1.5">
                {GENDER_OPTIONS.map((g) => (
                  <button key={g} onClick={() => setForm({ ...form, disc_gender: g })} className={`px-3 py-1.5 rounded-pill text-xs font-medium ${form.disc_gender === g ? "bg-s-coral text-white" : "bg-s-ink/5 dark:bg-white/5 text-s-ink/60"}`}>
                    {g === "female" ? "Women" : "Men"}
                  </button>
                ))}
              </div>
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-1.5">
                {TEXTURE_OPTIONS.map((t) => (
                  <button key={t} onClick={() => setForm({ ...form, disc_hair_texture: t })} className={`px-3 py-1.5 rounded-pill text-xs font-medium capitalize ${form.disc_hair_texture === t ? "bg-s-coral text-white" : "bg-s-ink/5 dark:bg-white/5 text-s-ink/60"}`}>
                    {t}
                  </button>
                ))}
              </div>
            ...
            <div>
              ...
              <div className="flex gap-1.5">
                {LENGTH_OPTIONS.map((l) => (
                  <button key={l} onClick={() => setForm({ ...form, disc_hair_length: l })} className={`px-3 py-1.5 rounded-pill text-xs font-medium capitalize ${form.disc_hair_length === l ? "bg-s-coral text-white" : "bg-s-ink/5 dark:bg-white/5 text-s-ink/60"}`}>
                    {l}
                  </button>
                ))}
              </div>
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-1.5">
                {FACE_SHAPE_OPTIONS.map((f) => (
                  <button key={f} onClick={() => setForm({ ...form, disc_face_shape: f })} className={`px-3 py-1.5 rounded-pill text-xs font-medium capitalize ${form.disc_face_shape === f ? "bg-s-coral text-white" : "bg-s-ink/5 dark:bg-white/5 text-s-ink/60"}`}>
                    {f}
                  </button>
                ))}
              </div>
