  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-2 mb-2">
        ...
        <h3 className="font-heading font-semibold text-sm text-s-ink dark:text-s-dm-text">Infill-Erinnerungen</h3>
      </div>

      {services.length === 0 ? (
        <p className="text-sm text-s-ink/40 dark:text-s-dm-text/40">Keine Nail-Services gefunden</p>
      ) : (
          ...
          {services.map((svc) => (
            ...
            >
              ...
              <div className="flex items-center gap-2">
                ...
                />
                <span className="text-xs text-s-ink/40 dark:text-s-dm-text/40">Tagen</span>
              </div>
            </div>
          ))}
        </div>
      ...
      {dueClients.length > 0 && (
        <div className="mt-4 p-3 rounded-card bg-s-amber-subtle dark:bg-s-amber/5 border border-s-amber/20">
          <p className="text-sm font-medium text-s-amber-text dark:text-s-amber flex items-center gap-1.5">
            <Users size={14} />
            {dueClients.length} Kunden fällig diese Woche
          </p>
        </div>
      )}
    </div>
