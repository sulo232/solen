...
export default function PackageManager({ salonId }: PackageManagerProps) {
  ...
  return (
    <div>
      ...
      {showForm && (
        <div className="bg-s-bg-surface dark:bg-s-dm-bg rounded-[16px] border border-s-ink/5 dark:border-white/5 p-4 mb-4 space-y-3">
          ...
          <div className="flex gap-2 justify-end">
            <button onClick={() => setShowForm(false)} className="px-3 py-1.5 text-xs text-s-ink/50 dark:text-s-dm-text/50">{t("cancel")}</button>
            <button
          ...
          {packages.map((pkg) => (
            <div key={pkg.id} className="flex items-center justify-between bg-s-bg-surface/50 dark:bg-s-dm-bg/50 rounded-[16px] border border-s-ink/5 dark:border-white/5 p-3">
              ...
              <div className="flex items-center gap-3">
                ...
                <button onClick={() => toggleActive(pkg)} aria-label={pkg.is_active ? t("deactivate") : t("activate")} className="text-s-ink/30 dark:text-s-dm-text/30 hover:text-s-coral transition-colors">
      ...
      {purchases.length > 0 && (
        <div>
          <h4 className="text-xs font-semibold text-s-ink/40 dark:text-s-dm-text/40 uppercase tracking-wide mb-2">{t("recentPurchases")}</h4>
