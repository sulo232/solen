    ...
    <div>
      <div className="flex items-center justify-between mb-4">
        <h3 className="font-heading font-bold text-sm text-s-ink dark:text-s-dm-text flex items-center gap-2">
          <Package size={14} className="text-s-coral" /> Service-Pakete
        </h3>
        <button
          onClick={() => setShowForm(!showForm)}
          className="flex items-center gap-1 px-3 py-1.5 rounded-button bg-s-coral text-white text-xs font-medium hover:bg-s-coral-hover transition-colors"
        >
          <Plus size={12} /> Neues Paket
        </button>
      </div>
      ...
      {showForm && (
        <div className="bg-s-bg-surface dark:bg-s-dm-bg rounded-card border border-s-ink/5 dark:border-white/5 p-4 mb-4 space-y-3">
          <input
            ...
            placeholder="Paketname (z.B. '5er Karte Haarschnitt')"
          ...
          <div className="grid grid-cols-3 gap-2">
            <div>
              <label className="text-[10px] text-s-ink/40 dark:text-s-dm-text/40">Sitzungen</label>
              <input
                type="number"
                min={1}
                value={form.sessions}
            ...
            <div>
              <label className="text-[10px] text-s-ink/40 dark:text-s-dm-text/40">Bonus</label>
              <input
                type="number"
                min={0}
            ...
            <div>
              <label className="text-[10px] text-s-ink/40 dark:text-s-dm-text/40">Preis (CHF)</label>
              <input
                type="number"
                min={0}
                step={5}
          ...
          <div className="flex gap-2 justify-end">
            <button onClick={() => setShowForm(false)} className="px-3 py-1.5 text-xs text-s-ink/50 dark:text-s-dm-text/50">Abbrechen</button>
            <button
      ...
      {/* Package list */}
      {packages.length === 0 ? (
        <p className="text-xs text-s-ink/30 dark:text-s-dm-text/30 text-center py-6">Noch keine Pakete erstellt</p>
      ) : (
            ...
            <div key={pkg.id} className="flex items-center justify-between bg-s-bg-surface/50 dark:bg-s-dm-bg/50 rounded-card border border-s-ink/5 dark:border-white/5 p-3">
              <div>
                ...
                <p className="text-xs text-s-ink/40 dark:text-s-dm-text/40">
                  {pkg.sessions} + {pkg.bonus_sessions} Bonus · {pkg.service_name} · {pkg.purchases_count} verkauft
                </p>
              </div>
              <div className="flex items-center gap-3">
                ...
                <button onClick={() => toggleActive(pkg)} className="text-s-ink/30 dark:text-s-dm-text/30 hover:text-s-coral transition-colors">
      ...
      {purchases.length > 0 && (
        <div>
          <h4 className="text-xs font-semibold text-s-ink/40 dark:text-s-dm-text/40 uppercase tracking-wide mb-2">Letzte Käufe</h4>
