...
export function BarbershopAboveGrid() {
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="flex items-center justify-between gap-4 px-4 py-3 bg-white/[.62] dark:bg-[rgba(38,32,26,.70)] backdrop-blur-[16px] saturate-[1.2] dark:backdrop-saturate-100 border border-white/55 dark:border-white/10 rounded-card shadow-warm-md" style={{ boxShadow: "var(--sh-md), inset 0 1px 0 rgba(255,255,255,.70)" }}>
        ...
        <Link
          ...
          className="shrink-0 flex items-center gap-1 px-3 py-1.5 rounded-btn bg-s-amber text-white text-xs font-body font-medium hover:bg-s-amber-hover transition-colors shadow-warm-sm"
        >
          <Zap size={12} />
          Walk-ins
        </Link>
    ...
    <section className="pt-12 pb-4">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        {FEATURED_BARBERS.map((barber) => (
          <div
            key={barber.id}
            className="rounded-card bg-white dark:bg-s-dm-surface border border-s-ink/5 dark:border-white/5 p-4 flex flex-col items-center text-center shadow-warm-sm hover:shadow-warm-xl hover:-translate-y-[5px] transition-all duration-250"
          >
