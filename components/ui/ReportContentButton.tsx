    ...
    <>
      ...
      {isOpen && (
        <div className="fixed inset-0 z-50 bg-s-ink/40 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white rounded-card shadow-glass w-full max-w-md p-6 relative animate-in fade-in zoom-in-95 duration-200">
            <button 
              onClick={() => !loading && setIsOpen(false)}
              className="absolute top-4 right-4 text-s-ink/40 hover:text-s-ink hover:bg-s-ink/5 p-1 rounded transition-colors"
            >
              <X size={20} />
            </button>
