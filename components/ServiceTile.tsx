...
export default function ServiceTile({ category, minPrice, isFavorite = false, locale = "de" }: ServiceTileProps) {
  ...
  return (
    <Link
      ...
      className="relative flex-shrink-0 flex flex-col items-center justify-center gap-2 w-28 h-28 rounded-card bg-white dark:bg-s-dm-surface shadow-card hover:shadow-card-hover hover:scale-[1.02] transition-all duration-200 p-3"
    >
      {isFavorite && (
