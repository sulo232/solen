...
- **V2-D15-3 era V3 dark teal trio** (retired V2-D48 2026-05-09): `#043338` brand teal + `#C2F0F1` pale teal + `#E1F4F4` brand subtle + `#0A6873` brand-mid. Replaced by Earthen Wellness Light emerald palette (also now retired — see below).
- **V2-D15-3 era V3 cat text colors** (retired V2-D48): cherry `#B5345A` (was Coiffeur text), magenta `#B50051` (was Nails text), sandy beige `#948565` (was Spa text). Replaced by V2-D70 cat mapping below.
...
- **V2-D48 Earthen Wellness Light emerald `#5C7765`/`#1F5C42`/`#1A8F5C`** (retired V2-D70 2026-05-18): emerald-forest brand color family. Replaced by V2-D70 forest `#3B7A57` then royal blue `#1638C4` (current).
- **V2-D60 Vibrancy Tune emerald + terracotta** (retired V2-D70 2026-05-18): `#1A8F5C` punched-up emerald + `#E0703D` warm terracotta + cream substrate `#FAF3E6`. Replaced by V2-D70 Aurex/Fresha warm-minimal pivot (royal blue + butter yellow + warm pearl, now pure white per V3-D94).
- **Peace Sans + Open Sauce One** (retired V2-D70 2026-05-18): V2-D42 typography pair. Replaced by Bricolage Grotesque + Hanken Grotesk (V3-D75).
- **V3-D86 → V3-D93 substrate experiments**: V3-D86 went pure white #FFFFFF (atmosphere wash retired). V3-D93 briefly flipped to subtle warm #FAF8F2. V3-D94 reverted to #FFFFFF — current locked substrate.

**Locked V3 brand (V2-D70 Aurex/Fresha warm-minimal pivot, 2026-05-18 — supersedes V2-D60):** royal blue `#1638C4` (`s-brand` — was emerald `#1A8F5C` V2-D60) + butter yellow `#FFC32B` (`s-accent` — was terracotta `#E0703D` V2-D60) + pure white substrate `#FFFFFF` (`s-bg.base` — V3-D94 reverted V3-D93's brief #FAF8F2 warm experiment). WHITE cards `#FFFFFF` (`s-bg.surface`) preserved from V2-D60 fix. Pale blue `#B8C4F0`, subtle blue `#E2E8FA`, brand-mid `#0F2BA0`, brand-deep `#08185E`. Accent soft butter `#FFE19F`, deep `#C9941F`. Forest green `#3B7A57` lives in `s-coral` legacy alias (backward-compat token, NOT a primary brand color). Bright accent butter `#F2D77B` (`s-butter`) for stat cards only. Love red `#CC4A60` (`s-love`) for heart-saved + sale chips (Airbnb pattern, V3-D85-semantic).

**⚠ Brand-color decision is currently OPEN** (as of 2026-05-21): user explicitly said "i dont think the royal blue was a good color choice" and explored alternatives via mockups (terracotta-led, walnut/Aesop, Grok-refined clay). Nothing locked yet. Production stays on V2-D70 royal blue + butter until a new direction is picked. Exploration mockups in `public/solen-brand-direction-*.html`. Council notes in `_audits/screenshots/poster-gradient/spec.md`.

**Color rule (V2-D49j 2026-05-10, partially-stale post-V2-D70):** the rule's STRUCTURE still applies (one color for actions, one for accents, no scatter) but the SPECIFIC color assignments are pre-V2-D70. Current production behavior: royal blue `s-brand` is the action color (CTAs, primary links, focus outlines, "see all" links). Butter yellow `s-accent` lives on logo dot + stars + ONE-OR-TWO highlighted words in h1 display text. Never on buttons. Love red `s-love` = heart-saved + sale chips only (universal semantic).

**Locked V3 categories (4, V2-D70 warm-minimal pivot tile colors retained from V2-D60):** Coiffeur `#FFE8D8`/`#E0703D` (peach + warm terracotta — kept for cat tile despite brand pivot), Barbershop `#EAE0D0`/`#2A1F18` (bone + ink), Nails `#D4DDC8`/`#A04A22` (sage-pale + terra-deep), Spa & Wellness `#D4F2E0`/`#0F6F44` (emerald-subtle + emerald-mid). Category text colors use the old V2-D60 emerald/terracotta hex for tile recognition — they're independent of the brand action color. **Note:** Grok's run-2 critique (2026-05-21) suggested unifying these to tints of a single warm palette instead of 4 distinct hues — open question for next brand iteration.

**Locked V3 typography (V3-D75 — supersedes V2-D42 Peace Sans + Open Sauce):** Bricolage Grotesque (display: hero h1, section h2s) + Hanken Grotesk (body/UI: paragraphs, microcopy, buttons). Both via Google Fonts (`@import url(...family=Bricolage+Grotesque:wght@400;500;600;700;800&family=Hanken+Grotesk:wght@400;500;600;700;800)`). Bricolage at 800 weight for h1, 700-800 for h2. Hanken at 400 for body, 600-700 for buttons. Tracking `-0.018em` on display fonts per V3-D72.

Full V3 spec lives in `_tasks/SOLEN_LIVE_TRUTH.md` (944 lines, V3-only after V2-D67-fu14 cleanup 2026-05-17): §1 brand, §2 categories, §5 typography, §5a pill rule, §5g atmosphere wash, §5h color philosophy, §13 Hero, §14 SearchBar, §16 Salon card. The 5,178-line pre-cleanup snapshot lives at `_tasks/archive/SOLEN_LIVE_TRUTH_v3-pre-cleanup-2026-05-17.archived.md` (don't grep — V2-D15-3 dark teal + Cooper + 6-cat era contamination). Operational playbook (pattern library + Fresha translation rules): `_rules/SOLEN_PATTERNS.md`. Decision log: `_tasks/V2_REBUILD_LOG.md` (latest entries V2-D71 / V2-D70 / V2-D68 / V2-D67-fu14 / V2-D67-fu13 / V2-D60 / V2-D49j / V2-D48 / V2-D42).

**Decision-log naming convention (locked 2026-05-18):** entries up to **V2-D71** use the legacy `V2-D##` prefix (the LOG was originally named during the V2→V3 rebuild — the prefix is the LOG name, not the design era). **From V3-D72 onward, new entries use the `V3-D##` prefix** to reflect the current era. Numerical sequence continues (V2-D71 → V3-D72). Filename kept as `V2_REBUILD_LOG.md` for now (renaming = 200+ reference sweep, not worth it).
