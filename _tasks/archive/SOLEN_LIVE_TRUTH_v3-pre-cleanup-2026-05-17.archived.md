# 🛑 ARCHIVED — Solen Live Truth (pre-cleanup snapshot, 2026-05-17)

> **THIS FILE IS RETIRED.** Do NOT grep this for current V3 state. It contains 4,200+ lines of V2-era contradictions: V2-D15-3 dark teal `#043338`, Cooper BT + Avant Garde Gothic typography, 6-cat colorways, 31-combo Republik library, frosted-glass pill on card text, V2-D48 pre-vibrancy palette, and dozens more retired specs that this snapshot never had cleaned up.
>
> **For the current V3 spec, see** `_tasks/SOLEN_LIVE_TRUTH.md` (944 lines, V2-D67-fu14 lock, no contradictions). The new doc covers everything: V2-D60 vibrancy palette, Peace Sans + Open Sauce One typography, V2-D55/V2-D64/V2-D67-fu12 atmosphere recipe, V2-D67-fu13 bare-text card lock, V2-D51 SearchBar Hybrid Hub, V2-D67 Hero (no eyebrow / no subtitle / no counter pill / no emoji).
>
> **Why archived (V2-D67-fu14 cleanup, 2026-05-17):** the doc accumulated 5+ override banners across major pivots (V2-D42, V2-D48, V2-D60, V2-D67) but the section bodies were never rewritten — only banners were added. Agents grep'd it and got stale answers (e.g. §V2-D41.2 frosted pill spec on cards, retired weeks ago, still cited as current). Audit found 186 contradictions, 16 gaps, 11 dead sections. Rather than patch in place, the user (Sulo) directed: "archive the v2 and keep only v3". Original doc preserved here for historical archeology only.

---

# Solen — Live Truth (V3, V2-D15-3 lock) — RETIRED 2026-05-17

> **Status:** RETIRED · **Locked:** 2026-05-07 (V2-D15-3 — V3 brand pivot to dark teal `#043338` + Cooper BT + Avant Garde Gothic + 4 categories) · **V3 Preview:** `public/solen-v2-republik-teal.html` (homepage) · `public/solen-v2-palette.html` (palette) · `public/solen-v2-combos.html` (31-combo grid) — serve via `npx serve public -p 4747`
>
...
> ⚠️ **V2-D67-fu13 (2026-05-17) — bare text below photo RE-LOCKED.** The V2-D41.2 frosted-glass pill is **RETIRED**. User explicitly rejected pill / wrapper / border / blur on the text region: "i just want text not covered or inside a pill jst text". The text region is now a permanent no-touch slot — depth comes from photo shadow + section panel opacity, NOT from wrapping text.
>
> ⚠️ **V2-D41 (2026-05-09) refined the card.** V2-D34's photo-overlay anatomy still applies; dimensions + hover + heart's saved state were updated. **See V2_REBUILD_LOG.md §V2-D41.3 to §V2-D41.5 for those deltas.** §V2-D41.2 (the pill) is retired per V2-D67-fu13.
>
> Quick deltas:
> - **§16.2 dimensions**: 160 mobile → **150 mobile** (180 desktop unchanged) — see updated §16.2 table.
> - **Text below photo**: **bare text** (Open Sauce One). 3 rows per V2-D60-cards-7 (Name h3 / Address · City / nextSlot · CHF · ★rating). 10px margin-top from photo, 2px horizontal padding. **NO surface, NO container, NO pill, NO border, NO backdrop-blur** around the text — ever. Depth in this section must come from photo shadow or section panel, not from text wrapping.
...
|photo to text gap |10px (`mt-[10px]` on the bare-text block — V2-D67-fu13 restored from §V2-D41.2 pill's 8px back to V2-D34's 10px)|
|text container    |**none — bare text only** (V2-D67-fu13 retired the V2-D41.2 frosted pill; see banner above §16.1)|
|total height      |~245px (150px photo + 10px gap + ~85px bare-text block on mobile)|
...
**12-15 cards loaded server-side per row (V2-D60.1, 2026-05-14 — bumped from 8-10 for fuller side-scrollable feel).** NOT infinite scroll within the row — user uses "Alle →" link in section header to see more (per §15). Marketplace surfaces benefit from "more to discover" density; 6-8 felt thin compared to Fresha / Airbnb visible-cards-on-scroll patterns.
