...
**2026-05-13 (V2-D58.0.1 — Phase 0 extensions complete: 14 sub-phases via parallel research agents, super-synthesis locked.)** — User requested deeper Phase 0 coverage before Phase 1 ("alot of subagents and alot of stuff"). Dispatched 14 parallel general-purpose research agents covering: 0.8 booking flow deep-dive (Fresha/Groomit/Swiss baseline) → 0.9 mobile UX patterns (8 platforms at 375×812) → 0.10 reviews+photos UX → 0.11 pricing presentation model → 0.12 search+filter UX (Fresha/Yelp/Booksy/Doctolib) → 0.13 onboarding flow → 0.14 notification+reminder UX → 0.15 adjacent platform booking (Booksy/StyleSeat/Doctolib/OpenTable) → 0.16 empty states+edge cases → 0.17 SEO patterns → 0.18 B2B groomer-side UX (Phase 6 prep) → 0.19 photography+brand identity → 0.20 loyalty/subscription deep-dive → 0.21 trust+safety mechanics. Each sub-phase produced a 280-540 line audit doc in `_audits/`. **Super-synthesis at `_audits/pet-grooming-scope-teardown-v2.md`** consolidates §11-§24 (one section per sub-phase with locked decisions) + §25 (full MVP feature spec ready for Phase 1+ execution: 9-service catalog, 5-required-field pet profile, 8-step booking flow w/ 5-6 returning user collapse, CHF 20 platform-default deposit, 24h free cancel window, defer-charge or upfront deposit A/B, OpenTable slot-pills on results cards, Doctolib 1-task-per-screen pacing, StyleSeat magic-link guest claim, Twint mandatory + Stripe Card, Resend email + eCall.ch SMS, Solen Abo v2 schema-aware in Phase 3 with 2 tiers CHF 39/79, 18% first-booking-only commission for Phase 6, hand-verified Basel groomers at launch with EFZ/SVBT badge differentiator, CHF 1M Berufshaftpflicht minimum, 5-promise Solen Versprechen "Geprüfte Hygiene · Tierfreundlich · Versichert · Faire Preise · 100% kostenlose Stornierung 24h"). Full schema delta SQL preview in §25 ready for Phase 3 execution. Phase 4 build checklist + Phase 5 SEO + Phase 6 B2B all spec'd. Estimated savings on Phase 4 build budget: 3-5 hrs UX discovery deferred. Total Phase 0 (v1+extensions) investment: ~10 hrs research + 14 audit deliverables + 1 master synthesis + dozens of architectural decisions locked. **Phase 1 (codebase audit) remains unblocked and now executes against richer scope spec.** Files created: `_audits/booking-flows.md`, `_audits/mobile-ux-patterns.md` + 8 screenshots in `_audits/pet-refs/mobile/`, `_audits/reviews-photos-ux.md`, `_audits/pricing-presentation.md`, `_audits/search-filter-ux.md`, `_audits/onboarding-flow.md`, `_audits/notifications-ux.md`, `_audits/adjacent-booking-patterns.md`, `_audits/empty-states-edge-cases.md`, `_audits/seo-patterns.md`, `_audits/b2b-groomer-side.md`, `_audits/photography-brand-patterns.md`, `_audits/loyalty-subscription-patterns.md`, `_audits/trust-safety-mechanics.md`, `_audits/pet-grooming-scope-teardown-v2.md` (super-synthesis). ·

**2026-05-13 (V2-D58.0 — PIVOT LOCKED: Solen → dog grooming, Basel-first. Phase 0 scope analysis complete.)** — Master pivot from human beauty salon booking to dog grooming booking, Fresha-style, Basel-first. Driver = (d) market signal + (c) design-loop fatigue. Market validation: NO CH-localized dog grooming marketplace exists. Tipaw.com is a breeder marketplace with side directory (not grooming-focused). Pawshake.ch + Rover.com/ch/ do pet-sitting only, no grooming. Individual CH groomers (cutnlove.ch, petwashbasel.ch, hundesalon-schnauz.ch, etc.) run Wix templates or WhatsApp-preferred booking — petwashbasel.ch explicitly says "TERMINE BITTE VORZUGSWEISE PER WHATSAPP ODER SMS." Open lane confirmed. Codebase audit (Phase 0 sub-step via Explore agent): ~65-70% structural reuse — booking engine, payments, geo, reviews, scheduling, auth, profile, checkout, F2 header structure all survive 1:1; the 30% rebuild is SalonCategory enum (6 beauty cats → 8 grooming services), Profile.hair_type (drop → pets.coat_type), Service.suitable_gender (drop → services.suitable_size_class), ~25 beauty-locked components under components-legacy/{coiffeur,barber,nail,spa}/, 6 category landing pages, brand identity, photography library. Phase 0 deliverable: `_audits/pet-grooming-scope-teardown.md` (10 sections covering market context, Fresha UX patterns to steal, pet platform competitor analysis, CH dog grooming baseline, pet-specific data model, service catalog MVP, booking flow spec, MVP feature scope, Phase 2 brand open questions, Phase 1 prerequisites table). Reference screenshots in `_audits/pet-refs/`: fresha-homepage.jpeg, rover-homepage.jpeg, pawshake-homepage.jpeg, tipaw-homepage.jpeg, cutnlove-homepage.jpeg, petwashbasel-homepage.jpeg. **Locked Phase 0 decisions** (defaults — user redirects override): MVP service catalog = 8 services (Bad / Bürsten / Vollverwöhnpaket / Schnitt / Krallenkürzen / Pfotenpflege / Ohrenreinigung / Zahnpflege) with size-class pricing (XS≤5kg / S 5-10 / M 10-25 / L 25-45 / XL 45+) on Schnitt/Voll/Deshedding; Pet profile = 5 required fields (name, species, breed, size_class, coat_type) + 4 optional (age, allergies, temperament, vaccinations free-text) + 1 photo (optional); Geographic launch = Basel + 5-10km radius first, Zürich after 100 bookings; Single-pet booking only in MVP; Salon-only (no mobile groomers) in MVP; Vaccination upload = optional free-text in MVP, structured enforcement v2; B2C-first, B2B groomer tools deferred to Phase 6. **Brand direction reference**: Fresha (warm consumer-marketplace, single-font RoobertPRO, 16px cards, 999px pills, yellow `#FFC00A` stars, `#0D0D0D` warm dark, `#F2F2F2` light bg, `cubic-bezier(0.85, 0, 0.15, 1)` 0.1-0.3s motion) > Rover (playful Bogart-serif h1 + Averta body) > Fluz (REJECTED — brutalist condensed wrong genre for pet-grooming warmth). Pet platforms default to BLUE accent (Rover #2E67D1, Pawshake #00AFED) — Solen's emerald could differentiate OR feel discordant; Phase 2 decision. Name decision (keep Solen vs rebrand) also Phase 2. Phase 1 prerequisites locked: new `pets` table + `bookings.pet_id` column, drop `profiles.hair_type` + `services.suitable_gender`, rename `salons.category` enum (delete coiffeur/barbershop/nails/spa/makeup/waxing values). Files created: `_audits/pet-grooming-scope-teardown.md`, `_audits/pet-refs/` (6 screenshots). Plan locked: `/Users/sulo/.claude/plans/immutable-shimmying-meerkat.md` (master pivot plan with 7 phases, Phase 0 deep plan, critical files table, risks per phase). **Phase 1 (codebase pivot audit) is unblocked.** ·

**2026-05-11 (V2-D56 — Fluz-pattern header lock: dark `#1F1714` bar + body bg matching + rounded main wrapper read as ONE continuous shape)** — User requested matching the Fluz top-of-page architecture after an extended iteration cycle (~12 wrong-direction guesses before landing). Final locked pattern: (1) **Header** = `fixed top-0` dark `#1F1714` (warm dark brown, user-picked from their reference swatch), `h-[71px]` (Fluz's exact desktop measurement), full width edge-to-edge, NO border-radius — flat at top + bottom. White wordmark + white nav links + emerald `s-brand` Anmelden pill preserved. (2) **Body bg** in `globals.css` changed from `transparent` (which let `body::before`/`body::after` atmosphere wash layers tint everything) to solid `#1F1714` matching the header. The wash layers still paint behind body's solid color but no longer leak into the rounded corner cutouts. (3) **Main wrapper** in `app/[locale]/layout.tsx`: `mt-[71px]` (starts exactly at header bottom, no overlap), `rounded-t-[40px]` (substantial radius for clearly visible curves — Fluz uses 15px which was too subtle at Solen's contrast, 40px reads dramatic), `bg-s-bg-base` cream (`#F5EBDD`) so content sections retain the V3 Earthen Wellness Light substrate. (4) **The visible effect**: dark header reads as one continuous shape with the corner cutouts — the dark color "dips down" with rounded curves into the cream content area below. **Critical insight learned across iterations**: rounded-bottom on the dark header (`rounded-b-[X]`) is NOT what creates this — it's rounded-top on the main wrapper below + matching dark body bg. The "header + curves are one part" mental model is mandatory: visually they're one shape, technically they're two elements (dark header at z-50, dark body bg behind cream main wrapper) coordinated to read as continuous. (5) **Atmosphere wash compatibility**: `body::before` / `body::after` wash layers retired effectively for the homepage since the solid dark body bg covers them. V2-D55 5-blob `AtmosphereBlobs.tsx` component on the page still paints over main's cream substrate via `position: fixed inset-0 z-0` so the soft tonal warmth survives in the cream area but the dark cutouts stay pure. (6) **Reverted in this same session as wrong**: rounded-bottom on header (multiple radius attempts), `rounded-t` on main with cream body (cutouts invisible), `rounded-t` on main with green gradient bg (user: "remove this green unnecessary"). Files patched: `app/[locale]/_components/layout/Header.tsx` (bg + h-[71px] + flex items-center + text colors white + emerald Anmelden), `app/globals.css` (body bg `transparent` → `#1F1714`), `app/[locale]/layout.tsx` (main `mt-[71px] rounded-t-[40px] bg-s-bg-base`). Reverted earlier in session: Hero.tsx rounded panel attempts, SectionHeader.tsx FeedZone variants. Public mockup that informed final landing: `public/solen-v2-fluz-clone.html` (kept, helped diagnose the misread). ·
