...
export async function POST(req: NextRequest) {
  ...
  const { shape, style, colors, length, material, skinTone, shotType } = body as {
    shape: string;
    length?: string;
    material?: string;
    style: string;
  ...
  if (!shape || !style || !colors) {
    return NextResponse.json({ error: "shape, style, and colors are required" }, { status: 400 });
  }
