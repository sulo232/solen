...
import { validateBody, adminDisputeActionSchema } from "@/lib/validations";
...
export async function PATCH(req: NextRequest) {
  ...
  const { data: validated, error: validationError } = validateBody(adminDisputeActionSchema, body);
  if (validationError) return NextResponse.json({ error: validationError.message }, { status: 400 });
  const { dispute_id, decision, admin_amount } = validated;
