...
export async function GET(req: NextRequest) {
  ...
  // Map to field names expected by NailBookingSteps + StationManager
  const defaults = { station_count: 4, has_uv_lamps: true, uv_lamp_count: 4, sterilization_buffer_minutes: 10 };
  const raw = data ?? defaults;
  return NextResponse.json({
    station: { ...raw, total_stations: raw.station_count },
  });
}
...
export async function PUT(req: NextRequest) {
  ...
  return NextResponse.json({ station: { ...stations, total_stations: stations.station_count } });
}
