...
export async function POST(req: NextRequest) {
  ...
  const { staff_member_id, service_ids } = body;

  if (!staff_member_id || !Array.isArray(service_ids)) {
    return NextResponse.json({ error: "staff_member_id and service_ids[] required" }, { status: 400 });
  }
