...
import { validateBody, priceOfferSchema } from "@/lib/validations";
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: validated, error: validationError } = validateBody(priceOfferSchema, body);
  if (validationError) return NextResponse.json({ error: validationError.message }, { status: 400 });
  const { description, amount_chf, photo_url } = validated;
...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const body = await req.json();
  if (!body.offer_id || !body.action || !["accept", "decline"].includes(body.action)) {
