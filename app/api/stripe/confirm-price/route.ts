...
export async function POST(req: NextRequest) {
  ...
  const { data: validated, error: valError } = validateBody(confirmPriceSchema, body);
  ...
  if (validated.final_price <= (booking.estimated_price ?? 0)) {
    ...
    const captureAmount = toRappen(validated.final_price);
    ...
    if (customerAuth?.user?.email) {
      await sendEmail({
        ...
        html: `<p>Deine Zahlung für den Termin bei <strong>${salon.name}</strong> wurde abgeschlossen.</p><p><strong>Endpreis:</strong> CHF ${validated.final_price.toFixed(2)}</p><p>Danke für deinen Besuch!</p>`,
      });
    }
    ...
    const approveUrl = `https://solen.ch/de/bookings/${validated.booking_id}/approve-increase`;
