...
import {
  ...
  Scissors, User, Sparkles, Waves, Palette, Zap
} from "lucide-react";
...
function Stars({ rating, size = "md" }: { rating: number; size?: "sm" | "md" }) {
  ...
  return (
    <span className={`flex gap-0.5 ${sz}`}>
      {[1, 2, 3, 4, 5].map((i) => (
        <span key={i} className={i <= Math.round(rating) ? "text-coral" : "text-gray-200"}>★</span>
      ))}
    </span>
  );
}
              ...
              <div className="lg:sticky lg:top-24">
                {!calendarOpen ? (
                  <button
                    ...
                    className="w-full py-4 rounded-card bg-coral text-white font-semibold text-base hover:bg-coral/90 transition-colors shadow-coral-glow"
                  >
                    Jetzt buchen
                  </button>
                ) : (
