...
export default function SalonReviewsPage() {
  ...
  return (
    <DashboardLayout>
      <div className="mb-6">
        <h1 className="font-heading text-2xl text-s-ink">Bewertungen</h1>
            ...
            >
              ...
              <div className="flex items-start justify-between mb-2">
                ...
                <button
                  ...
                  className="text-s-ink/30 hover:text-s-coral:text-s-coral p-1 transition-colors"
                  ...
                  {respondingTo === r.id ? (
                    <div className="space-y-2">
                      <textarea
                        ...
                        className="w-full px-3 py-2 rounded-btn border border-s-ink/10 bg-white text-s-ink text-xs focus:outline-none focus:border-s-coral resize-none"
                      />
                      <div className="flex gap-2 items-center">
                        ...
                        <button
                          ...
                          className="px-3 py-1.5 rounded-btn border border-s-ink/10 text-s-ink/40 text-xs"
                        >
                          Abbrechen
                        </button>
                        <button
              ...
              {/* Flagging form */}
              {flagging === r.id && (
                <div className="space-y-2 mt-3 p-3 bg-red-50 rounded-btn border border-red-100">
                  <p className="text-xs font-medium text-red-800">Warum meldest du diese Bewertung?</p>
                  <textarea
                    rows={2}
                    maxLength={250}
                    ...
                    className="w-full px-3 py-2 rounded-btn border border-red-200 bg-white text-s-ink text-xs focus:outline-none focus:border-red-400 resize-none"
                  />
                  <div className="flex gap-2 items-center justify-end mt-2">
                    <button
                      onClick={() => { setFlagging(null); setFlagReason(""); }}
                      className="px-3 py-1.5 rounded-btn border border-s-ink/10 text-s-ink/60 text-xs"
                    >
                      Abbrechen
                    </button>
                    <button
