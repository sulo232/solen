...
import {
  Search, Upload, Play, CheckCircle, XCircle, Trash2, Eye,
  Image as ImageIcon, Video, RefreshCw, Sparkles, ArrowUpDown,
  AlertTriangle, ExternalLink, GripVertical,
} from "lucide-react";
...
function StockImportTab() {
  ...
  const handleImportSelected = async () => {
    ...
    // Photos are already in the search results — add them to staging
    const selectedPhotos = photos.filter((p) => selected.has(p.id));
    let imported = 0;
    for (const photo of selectedPhotos) {
      try {
        // We use the staging API — create them individually
        const res = await fetch("/api/admin/discovery/staging", {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
  ...
  const handleBulkImport = async () => {
    ...
    try {
      ...
      const data = await res.json();
      alert(`Imported ${data.imported} photos to staging`);
    } finally {
...
function TikTokImportTab() {
  ...
  return (
    <div className="space-y-4 max-w-2xl">
      <p className="text-sm text-s-ink/60 dark:text-s-dm-text/60">Paste TikTok video URLs, one per line. AI will auto-categorize each video.</p>
      <select
        value={category}
        onChange={(e) => setCategory(e.target.value as DiscoveryCategory)}
        className="px-3 py-2.5 rounded-button bg-s-bg-sunken dark:bg-s-dm-surface border border-s-ink/10 dark:border-white/5 text-sm"
      ...
      {result && (
        <div className="flex gap-4 text-sm">
          <span className="text-green-600 flex items-center gap-1"><CheckCircle size={14} /> {result.imported} imported</span>
          {result.failed > 0 && <span className="text-red-500 flex items-center gap-1"><XCircle size={14} /> {result.failed} failed</span>}
        </div>
      )}
    </div>
  );
  ...
  return (
    <div className="space-y-4 max-w-lg">
      ...
      {uploadedItem && !analyzing && (
        <div className="flex items-center gap-3 p-3 rounded-card bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800">
          <CheckCircle size={18} className="text-green-600" />
          <span className="text-sm text-green-700 dark:text-green-300">Uploaded and published successfully</span>
        </div>
      )}
    </div>
  );
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap gap-3 items-center">
        ...
        {items.length > 0 && (
          <>
            <button onClick={selectAll} className="px-3 py-2 rounded-button bg-s-ink/5 dark:bg-white/5 text-sm">
              {selected.size === items.length ? "Deselect All" : "Select All"}
            </button>
            {selected.size > 0 && (
              <>
                <button onClick={() => handleAction("approve")} disabled={processing} className="px-3 py-2 rounded-button bg-green-600 text-white text-sm font-medium flex items-center gap-1.5 disabled:opacity-50">
                  <CheckCircle size={14} /> Approve {selected.size}
                </button>
                <button onClick={() => handleAction("reject")} disabled={processing} className="px-3 py-2 rounded-button bg-red-500 text-white text-sm font-medium flex items-center gap-1.5 disabled:opacity-50">
    ...
    <div ref={setNodeRef} style={style} className="relative rounded-card overflow-hidden bg-white dark:bg-s-dm-surface border border-s-ink/5 dark:border-white/5">
      <div className="aspect-[3/4] relative bg-s-ink/5 dark:bg-white/5">
        ...
        <div className="absolute top-2 left-2">
          <button {...attributes} {...listeners} className="w-7 h-7 rounded-full bg-s-ink/50 flex items-center justify-center cursor-grab active:cursor-grabbing touch-none">
            <GripVertical size={14} className="text-white" />
          </button>
        </div>
        <div className="absolute top-2 right-2 flex gap-1">
          <button onClick={() => onArchive(item.id)} className="w-6 h-6 rounded-full bg-s-ink/50 flex items-center justify-center hover:bg-red-600 transition-colors">
        ...
        {items.map((item) => (
          <div key={item.id} className="rounded-card bg-white dark:bg-s-dm-surface border border-s-ink/5 dark:border-white/5 overflow-hidden">
            ...
            <div className="p-3 space-y-2">
              <div className="flex items-center gap-1">
                <span className="text-[10px] px-1.5 py-0.5 rounded-pill bg-s-coral/10 text-s-coral font-medium">{item.category}</span>
                <span className="text-[10px] px-1.5 py-0.5 rounded-pill bg-red-50 text-red-600 font-medium">{item.content_type}</span>
              </div>
              {item.flag_reason && (
                <p className="text-xs text-red-500 flex items-center gap-1"><AlertTriangle size={12} /> {item.flag_reason}</p>
              )}
              <div className="flex gap-2">
                <button onClick={() => handleAction(item.id, "approve")} className="flex-1 py-2 rounded-button bg-green-600 text-white text-sm font-medium flex items-center justify-center gap-1">
                  <CheckCircle size={14} /> Approve
                </button>
                <button onClick={() => handleAction(item.id, "remove")} className="flex-1 py-2 rounded-button bg-red-500 text-white text-sm font-medium flex items-center justify-center gap-1">
