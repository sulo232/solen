    ...
    <motion.div variants={itemVariants} className="bg-white rounded-card border border-gray-100 shadow-card p-4">
          ...
          {(["de", "en"] as const).map((l) => (
            <button
              ...
              className={`px-2.5 py-1 text-[10px] font-medium transition-colors ${
                locale === l ? "bg-teal text-white" : "text-dark/40"
              }`}
            >
              {l.toUpperCase()}
            </button>
      ...
      {row.content_type === "text" && currentValue.length > 80 ? (
        <textarea
          ...
          rows={3}
          className="w-full px-3 py-2.5 rounded-button border border-gray-200 text-sm font-body text-dark focus:outline-none focus:border-teal resize-y"
        />
      ) : (
        <input
          ...
          value={currentValue}
          onChange={(e) => setCurrentValue(e.target.value)}
          className="w-full px-3 py-2.5 rounded-button border border-gray-200 text-sm font-body text-dark focus:outline-none focus:border-teal"
        />
      )}
      ...
      <div className="flex items-center justify-between mt-2">
        ...
        <button
          onClick={handleSave}
          disabled={saving}
          className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-button bg-teal text-white text-xs font-medium disabled:opacity-50 hover:bg-teal/90 transition-colors"
        >
