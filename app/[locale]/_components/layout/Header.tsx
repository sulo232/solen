      ...
      <div className="mx-auto flex max-w-[1280px] items-center gap-2.5 px-4 md:gap-6 md:px-8">
        ...
        <nav
          ...
          className={cn(
            ...
            // 56px fade (was 32px) → mid-word truncation reads as intentional
            // gradient instead of broken-looking cut on narrow mobile.
        ...
        >
          {CATEGORIES.map((c) => (
            <Link
              ...
              href={`/${locale}${c.href}`}
              className={cn(
                "relative shrink-0 whitespace-nowrap font-body text-[14px] font-medium text-s-ink-2",
                "transition-colors duration-200 ease-glide hover:text-s-ink",
                // V2-D62 (2026-05-15): Apple liquid-glass pill blooms BEHIND the text on hover.
                // Same recipe as the SalonCard pills (V2-D61-fu) — white tint at 30% alpha,
                // 22px backdrop blur + 1.7 saturate, inset top-edge highlight, no separate border.
                // The pseudo uses negative inset so the pill extends past the text without
                // adding padding to the link (layout stays exactly as-is at rest).
                // Overshoot-eased scale (0.6 → 1) gives the pill a soft "settle" on entry.
