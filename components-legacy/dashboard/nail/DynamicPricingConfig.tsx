...
export default function DynamicPricingConfig({ salonId }: { salonId: string }) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="p-3 rounded-[16px] bg-s-bg-surface overflow-x-auto">
        <p className="text-xs text-s-ink/40 mb-2">{t("pricing_overview")}</p>
      ...
      {/* New rule form */}
      {newRule && (
        <div className="p-3 rounded-[16px] border border-s-coral/20 bg-white space-y-3">
      ...
      <div className="space-y-2">
        {rules.map((rule) => (
          <div key={rule.id} className="flex items-center justify-between p-3 rounded-[16px] border border-s-ink/5 bg-white">
            <div>
        ...
        {rules.length === 0 && !newRule && (
          <p className="text-center text-sm text-s-ink/30 py-6">{t("pricing_empty")}</p>
        )}
      </div>
    </div>
  );
